...
{
    public partial class AktifRandevu : UserControl
    {
        private SqlCommand _command;
        private List<RandevuDto> randevular=new List<RandevuDto>(); 
        public AktifRandevu()
        {
            InitializeComponent();
        }
        ...
        private void GetRandevular()
        {
            ...
            while (reader.Read())
            {
                var randevu = new RandevuDto()
                {
                    ...
                    Durum = (reader[5].ToString() == "true") ? "Onaylı" : "Onaylanıyor",
                    HastaAdı = HastaDto.HastaAd
                };
                randevular.Add(randevu);

            }
            Datagridaktirandevu.DataSource = randevular;
        }
